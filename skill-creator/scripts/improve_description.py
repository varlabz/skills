...
by calling `pi -p` as a subprocess (same auth pattern as run_eval.py —
uses the session's pi agent auth, no separate API key needed).
"""
...
def improve_description(
    ...
    """Call pi agent to improve the description based on eval results."""
    failed_triggers = [
    ...
    prompt = f"""You are optimizing a skill description for a pi agent skill called "{skill_name}". A "skill" is sort of like a prompt, but with progressive disclosure -- there's a title and description that pi sees when deciding whether to use the skill, and then if it does use the skill, it reads the .md file which has lots more details and potentially links to other resources in the skill folder like helper files and scripts and additional documentation or examples.
